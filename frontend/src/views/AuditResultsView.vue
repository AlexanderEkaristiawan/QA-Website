  ...
  <div v-else class="space-y-8">
    <div class="flex items-start justify-between flex-wrap gap-4">
      <div>
        <button
          @click="goBack"
          class="text-sm text-indigo-600 hover:text-indigo-700 mb-2 flex items-center gap-1"
        >
          ← Back to Project
        </button>
        <div class="flex items-center gap-3">
          ...
          <span
            ...
            :class="
              audit.crawlMode === 'extension' ||
              audit.crawlMode === 'extension-instant'
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{
            audit.timestamp
              ? (audit.timestamp as any).toDate?.().toLocaleString()
              : "Processing..."
          }}
        </p>
      </div>
        ...
        <span
          ...
          :class="{
            ...
            'bg-red-100 text-red-800':
              audit.status === 'failed' || audit.status === 'auth-failed',
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="card p-5">
        ...
        <p
          ...
          :class="getScoreColor(audit.summaries?.performance?.performance ?? 0)"
        >
          {{ audit.summaries?.performance?.performance ?? 0
          }}<span class="text-base font-normal text-gray-400">/100</span>
        </p>
      ...
      <div class="card p-5">
        ...
        <div class="flex items-baseline gap-2">
          <span class="text-3xl font-bold text-red-600">{{
            audit.summaries?.security?.highAlerts ?? 0
          }}</span>
          <span class="text-xs text-gray-500"
            >high, {{ audit.summaries?.security?.mediumAlerts ?? 0 }} med</span
          >
        </div>
      ...
      <div class="card p-5">
        ...
        <p class="text-3xl font-bold text-gray-900">
          {{ audit.summaries?.seo?.pageCount ?? pages.length }}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          {{ audit.summaries?.seo?.totalErrors ?? 0 }} total issue(s)
        </p>
      </div>
      ...
      <div class="card p-5">
        ...
        <p class="text-3xl font-bold text-amber-600">
          {{ audit.summaries?.security?.missingHeaders?.length ?? 0 }}
        </p>
    ...
    <div class="card overflow-hidden">
      ...
      >
        <h3 class="text-base font-bold text-gray-900">
          📄 Crawled & Audited Pages ({{ pages.length }})
        </h3>
        <span v-if="loadingPages" class="text-xs text-gray-400"
          >Loading sub-collection...</span
        >
      </div>
      ...
      >
        {{
          loadingPages
            ...
            : "No page records stored for this audit yet."
        }}
      </div>
        ...
        >
          <div class="flex items-start justify-between flex-wrap gap-2">
            <div class="space-y-1 max-w-2xl">
              <div class="flex items-center gap-2">
                <span
                  ...
                  :class="
                    p.source === 'extension' || p.source === 'extension-instant'
            ...
            <div class="flex items-center gap-4 text-xs">
              <div
                v-if="typeof p.loadTimeMs === 'number' && p.loadTimeMs > 0"
                class="text-right"
              >
