...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    if (!title || !steps) {
      return jsonResponse(400, { error: 'title and steps are required' }, origin)
    }

    const stepsText = Array.isArray(steps) ? steps.map((s: string, i: number) => `  ${i + 1}. ${s}`).join('\n') : steps
...
Return ONLY valid TypeScript code with no markdown fences, no explanations — just the raw .ts file content.`
    ...
    return jsonResponse(200, { script }, origin)
  } catch (err: any) {
