...
async function fetchPage(url, authSettings) {
    const headers = {
...
async function isAllowedByRobots(baseUrl, path) {
    try {
        ...
        const { data } = await axios_1.default.get(robotsUrl, { timeout: 5000 });
        ...
        for (const line of lines) {
            ...
            if (trimmed.toLowerCase().startsWith('user-agent:')) {
                ...
                if (inQABot)
                    disallowedQA.push(disPath);
                if (inAll)
                    disallowedAll.push(disPath);
            }
        }
...
function parsePage(url, html, statusCode) {
    ...
    if (!title)
        ...
        issues.push({ type: 'title-length', description: `Title length is ${title.length} chars (ideal: 30-60)`, severity: 'Minor' });
    const metaDescMatch = html.match(/<meta\s+name=["']description["'][^>]*content=["']([^"']*)["']/i)
        || html.match(/<meta\s+content=["']([^"']*)["'][^>]*name=["']description["']/i);
    const metaDescription = metaDescMatch ? metaDescMatch[1].trim() : '';
    if (!metaDescription)
        issues.push({ type: 'missing-meta-description', description: 'Page is missing meta description', severity: 'Major' });
    else if (metaDescription.length < 70 || metaDescription.length > 160)
        issues.push({ type: 'meta-desc-length', description: `Meta description length is ${metaDescription.length} chars (ideal: 70-160)`, severity: 'Minor' });
    const h1Matches = html.match(/<h1[^>]*>[\s\S]*?<\/h1>/gi) || [];
    const h1Count = h1Matches.length;
    if (h1Count === 0)
        issues.push({ type: 'missing-h1', description: 'Page has no <h1> tag', severity: 'Major' });
    else if (h1Count > 1)
        issues.push({ type: 'multiple-h1', description: `Page has ${h1Count} <h1> tags (should have exactly 1)`, severity: 'Minor' });
    const imgTags = html.match(/<img[^>]+>/gi) || [];
    const missingAlt = imgTags.filter(img => !img.match(/alt=["'][^"']*["']/i) || img.match(/alt=["']['"]]/i)).length;
    if (missingAlt > 0)
        issues.push({ type: 'missing-alt', description: `${missingAlt} image(s) missing or empty alt attribute`, severity: 'Major' });
